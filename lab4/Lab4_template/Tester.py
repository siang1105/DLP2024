...
class Dataset_Dance(torchData):
    ...
    def __getitem__(self, index):
        ...
        imgs_tensor = stack(imgs)
        labels_tensor = stack(labels)
...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for i in range(1, label.shape[0]):
            ...
            decoder_input = self.Decoder_Fusion(frame_encoding, pose_encoding, z)
            generated_frame = self.Generator(decoder_input)
            
            decoded_frame_list.append(generated_frame.cpu())
            label_list.append(pose_encoding.cpu())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device',        type=str, choices=["cuda:7", "cpu"], default="cuda:7")
